    ...
    $.ajax({
      ...
      success: function(gistdata) {
        ...
        for (var i = 0; i < content["recipies"].length; i++) {
          $('#main').append('<div class="recept resize" style="background: url(' + content.recipies[i].photo + '); background-size: cover;"><a class="recept retext" href="?id=' + content.recipies[i].id + '&file=' + content.recipies[i].file + '&p=' + content.recipies[i].photo + '">' + content.recipies[i].name + '</a></div>');
        }
      },
      error: function(e) {
        alert("Error");
